...
const startWorker = (senders: { telegram: any, twilio: any, prisma: any }) => {
    ...
    if (!task) { activeWorkers--; return; }
    const worker = new Worker(path.resolve(__dirname, "queueWorker.js"), {
        workerData: { task, senders: { hasTelegram: !!senders.telegram, hasTwilio: !!senders.twilio } }
    });
    workers.push(worker);
...
const enqueue = (task: { alert: Alert & { user: User }, price: number }, senders: { telegram: any, twilio: any, prisma: any }) => {
    queue.push(task);
    startWorker(senders);
};

const enqueueBatch = (tasks: { alert: Alert & { user: User }, price: number }[], senders: { telegram: any, twilio: any, prisma: any }) => {
